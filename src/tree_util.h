...
public:
    ...
    BinTree(TreeNode* root) : root(root) {}
    ...
    void insert(const T& val, Args...more) {
        if (!root) {
            ...
            for (;;) {
                if (val == p->val) return;
                TreeNode*& pchild = val < p->val ? p->left : p->right;
                if (!pchild) {
                    pchild = new TreeNode(val);
                    break;
                }
                p = pchild;
            }
    ...
    display_rows get_row_display() const {
        ...
        // its TreeNode's val, created using a std::stringstream object.
    ...
    vector<string> row_formatter(const display_rows& rows_disp) const {
        ...
        // First find the maximum val string length and put it in cell_width
        s_t cell_width = 0;
        ...
        for (s_t r = 0; r < row_count; ++r) {
            ...
            for (s_t c = 0; c < row_elem_count; ++c) {
                ...
                if (cd_row[c].present) {
                    ...
                    // Try to pad the left and right sides of the val string
            ...
            // A row of spaced-apart val strings is ready, add it to the result vector
            ...
            // to "connect" two rows' TreeNode val strings.
